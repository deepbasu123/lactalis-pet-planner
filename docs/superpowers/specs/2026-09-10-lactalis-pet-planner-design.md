...
- Workspace: `DEFAULT` profile. Catalog/schema: `main.lactalis_pet_planner`.
...
└── Unity Catalog: main.lactalis_pet_planner  ── base tables + projection_snapshot
         │
...
## 3. Data model (UC Delta, `main.lactalis_pet_planner`)
...
- App SP grants: USE CATALOG + USE SCHEMA + SELECT + MODIFY on `main.lactalis_pet_planner`, CAN_USE on the warehouse, CAN RUN on the Genie space.
