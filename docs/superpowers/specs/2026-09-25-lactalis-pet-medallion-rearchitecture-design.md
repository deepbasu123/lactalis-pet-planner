...
Deploy a working copy to `DEFAULT` as a **new app `lactalis-pet-planner-v2`** with a
...
- **Recursive CTE works on DEFAULT's serverless warehouse** and computes the
...
## 4. Medallion layout (`main`, schema-per-layer)
